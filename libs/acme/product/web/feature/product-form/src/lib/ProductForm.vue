...
const errors = computed(() => {
    ...
    if (!form.value.sku) next.sku = 'SKU is required';
    if (String(form.value.sku ?? '').length > 64) next.sku = 'SKU must be 64 characters or less';
    if (form.value.sku && !new RegExp('^[a-z0-9]+(?:-[a-z0-9]+)*$').test(String(form.value.sku))) {
        next.sku = 'SKU is not in the expected format';
    }
    return next;
});
